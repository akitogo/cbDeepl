...
component {
	...
	// Module Entry Point
	this.entryPoint			= "cbDeepl";
	// Model Namespace
	this.modelNamespace		= "cbDeepl";
	// CF Mapping
	this.cfmapping			= "cbDeepl";
	// Auto-map models
	...
	*/
	function onLoad(){
		var DeeplApiKey = controller.getSetting( "DeeplApiKey",false,'' );
		if(DeeplApiKey eq '')
			throw(message="The DeeplApiKey may not be empty", detail="Please set it in your /config/Coldbox.cfc config file"); 

		// Map cbDeepl
		binder.map( "DeeplClient@cbDeepl" )
			.to( "#moduleMapping#.models.DeeplClient" )
			.initArg( name="apiKey", 			value=DeeplApiKey );
